...
function RankingView({ review }: { review: Review }) {
  if (!review.rankingItems || review.rankingItems.length === 0) return null;

  return (
    <div className="space-y-8 md:space-y-16">
      {/* 1. Quick Ranking (Selo MunizTech) */}
      {review.quickRanking && (
        <div className="bg-slate-900 rounded-[32px] md:rounded-[48px] p-6 md:p-12 shadow-2xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-secondary/10 blur-[100px] -mr-32 -mt-32" />
          <h3 className="text-white font-black uppercase tracking-widest text-xs md:text-sm mb-8 flex items-center gap-3">
             <Trophy className="w-5 h-5 text-yellow-400" /> Veredito Rápido: O que comprar?
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 relative z-10">
            {Object.entries(review.quickRanking).map(([key, value]) => {
              if (!value) return null;
              const labels: Record<string, { label: string; color: string; icon: string }> = {
                best_overall: { label: '🏆 Melhor Geral', color: 'text-yellow-400', icon: '🏆' },
                best_value: { label: '💰 Custo-Benefício', color: 'text-green-400', icon: '💰' },
                best_premium: { label: '💎 Escolha Premium', color: 'text-blue-400', icon: '💎' },
                best_alternative: { label: '🔄 Alternativa', color: 'text-slate-400', icon: '🔄' }
              };
              const cfg = labels[key] || { label: key, color: 'text-white' };
              return (
                <div key={key} className="bg-white/5 backdrop-blur-sm p-5 rounded-2xl border border-white/10">
                  <div className={`text-[10px] font-black ${cfg.color} uppercase tracking-widest mb-2`}>{cfg.label}</div>
                  <div className="text-white font-black text-sm md:text-base leading-tight">{value}</div>
                </div>
              );
            })}
          </div>
      ...
      <div className="space-y-8">
        <h3 className="text-2xl md:text-4xl font-black text-on-surface uppercase tracking-tight flex items-center gap-3">
          <Star className="w-8 h-8 text-secondary fill-current" /> A Lista Completa
        </h3>
        
        <div className="space-y-6 md:space-y-10">
          {review.rankingItems.map((item, idx) => {
            const isTop3 = idx < 3;
            const medals = ["🥇", "🥈", "🥉"];
            
            return (
              <div key={idx} className={`bg-white rounded-[32px] md:rounded-[48px] p-6 md:p-10 border-2 transition-all hover:shadow-2xl flex flex-col md:flex-row gap-6 md:gap-10 items-center relative overflow-hidden ${isTop3 ? 'border-secondary/20 shadow-xl' : 'border-surface-container-high shadow-sm'}`}>
                {/* Badge de Posição */}
                <div className="absolute top-6 left-6 md:top-8 md:left-8">
                   <div className={`w-12 h-12 md:w-16 md:h-16 rounded-full flex items-center justify-center font-black text-lg md:text-2xl shadow-lg border-4 border-white ${isTop3 ? 'bg-secondary text-white' : 'bg-slate-200 text-slate-600'}`}>
                      {isTop3 ? medals[idx] : idx + 1}
                   </div>
                </div>

                {/* Imagem do Produto */}
                <div className="w-full md:w-1/3 aspect-square bg-surface-container-low rounded-[24px] md:rounded-[32px] flex items-center justify-center p-8 group">
                   <img src={item.image} alt={item.name} className="max-h-full object-contain group-hover:scale-110 transition-transform duration-500" />
                </div>
                ...
                <div className="flex-1 space-y-4 md:space-y-6 text-center md:text-left w-full">
                   <div className="space-y-2">
                      <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                         <h4 className="text-xl md:text-3xl font-black text-on-surface uppercase tracking-tighter">{item.name}</h4>
                         <div className="px-3 py-1 bg-secondary text-white rounded-lg text-xs font-black flex items-center gap-1.5">
                            <Star className="w-3.5 h-3.5 fill-current" /> {item.rating}
                         </div>
                      </div>
                      <div className="text-xl md:text-2xl font-black text-secondary">{item.price}</div>
                   </div>

                   {/* Benefícios */}
                   <div className="space-y-3">
                      <div className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Por que escolher este?</div>
                      <div className="flex flex-wrap justify-center md:justify-start gap-2">
                         {item.benefits?.map((b, bi) => (
                           <span key={bi} className="px-4 py-2 bg-green-50 text-green-700 rounded-xl text-xs font-black flex items-center gap-2 border border-green-100">
                   ...
                   <a 
                     ...
                     className="inline-flex w-full md:w-auto px-10 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest text-sm items-center justify-center gap-3 hover:bg-secondary transition-all shadow-xl hover:scale-105 active:scale-95"
                   >
      ...
      {/* 3. CTA Final */}
      <div className="bg-secondary rounded-[32px] md:rounded-[48px] p-8 md:p-16 text-center space-y-6 md:space-y-8 relative overflow-hidden shadow-2xl">
        ...
        <p className="text-white/80 font-label-bold text-sm md:text-lg max-w-xl mx-auto relative z-10">
          Confira todas as ofertas monitoradas em tempo real pelo MunizTech e economize agora.
        </p>
        <Link
          to="/ofertas"
          className="inline-flex px-8 md:px-12 py-4 md:py-5 bg-white text-secondary rounded-2xl font-black uppercase tracking-widest text-sm md:text-lg items-center justify-center gap-3 transition-transform hover:scale-105 active:scale-95 shadow-xl relative z-10"
        >
          Ver Todas as Ofertas <ArrowRight className="w-5 h-5" />
        </Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-surface min-h-screen pb-32 font-sans overflow-x-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 mb-12 md:mb-20">
        {review.type === 'ranking' ? (
          /* Header de Ranking Premium */
          <div className="relative rounded-[24px] md:rounded-[48px] overflow-hidden shadow-2xl bg-slate-900 flex flex-col items-center justify-center p-8 md:p-20 text-center space-y-6">
             <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20" />
             <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-transparent to-transparent" />
             <div className="relative z-10 space-y-4">
                <div className="inline-flex px-4 py-1.5 bg-secondary text-white rounded-full font-black text-[10px] uppercase tracking-[0.2em] shadow-lg">
                  🏆 Ranking MunizTech
                </div>
             ...
             {review.image && (
               <img src={review.image} className="relative z-10 max-w-full max-h-[300px] object-contain drop-shadow-[0_20px_50px_rgba(255,255,255,0.1)]" alt={review.title} />
             )}
          </div>
